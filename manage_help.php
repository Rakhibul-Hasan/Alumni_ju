...
<?php
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM help_topics where id=".$_GET['id'])->fetch_array();
	foreach($qry as $k =>$v){
		$$k = $v;
	}
}
...
<script>
	$('.text-jqte').jqte();
	$('#manage-help').submit(function(e){
		e.preventDefault()
		start_load()
		$.ajax({
			url:'admin/ajax.php?action=save_help',
			method:'POST',
			data:$(this).serialize(),
