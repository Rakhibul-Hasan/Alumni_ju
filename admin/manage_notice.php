...
<?php
if(isset($_GET['id'])){
$qry = $conn->query("SELECT * FROM notice where id= ".$_GET['id']);
			...
			<div class="card-body">
				<form action="" id="manage-notice">
					...
					<div class="form-group row">
						<div class="col-md-5">
							<label for="" class="control-label">notice Title</label>
...
<script>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					...
					setTimeout(function(){
						location.href = "index.php?page=notice"
					},1500)
  ...
  var drop;
  addnoticeHandler(window, 'load', function() {
    ...
    addnoticeHandler(drop, 'dragover', cancel);
    addnoticeHandler(drop, 'dragenter', cancel);

    addnoticeHandler(drop, 'drop', function(e) {
      e = e || window.notice; // get window.notice if e argument missing (in IE)   
      if (e.prnoticeDefault) {
        e.prnoticeDefault();
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        //attach notice handlers here...

        reader.readAsDataURL(file);
        addnoticeHandler(reader, 'loadend', function(e, file) {
          var bin = this.result;
        ...
        }.bindTonoticeHandler(file));
      }
      return false;

    });

    Function.prototype.bindTonoticeHandler = function bindTonoticeHandler() {
      var handler = this;
...
function addnoticeHandler(obj, evt, handler) {
  if (obj.addnoticeListener) {
    // W3C method
    obj.addnoticeListener(evt, handler, false);
  } else if (obj.attachnotice) {
    // IE method.
    obj.attachnotice('on' + evt, handler);
  } else {
    // Old school method.
    obj['on' + evt] = handler;
  }
