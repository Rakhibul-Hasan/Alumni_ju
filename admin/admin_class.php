...
Class Action {
	...
	function save_help(){
		...
		if(empty($id)){
		$data .= ", user_id = '{$_SESSION['login_id']}' ";
			$save = $this->db->query("INSERT INTO help_topics set ".$data);
		}else{
			$save = $this->db->query("UPDATE help_topics set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_help(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM help_topics where id = ".$id);
		if($delete){
			return 1;
		}
	}
	function save_comment(){
		...
		if(empty($id)){
			...
			$data .= ", user_id = '{$_SESSION['login_id']}' ";
			$save = $this->db->query("INSERT INTO help_comments set ".$data);
		}else{
			$save = $this->db->query("UPDATE help_comments set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_comment(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM help_comments where id = ".$id);
		if($delete){
			return 1;
		}
	}
...
function save_notice(){
	...
	if(empty($id)){
		...
		$save = $this->db->query("INSERT INTO notice set ".$data);
	}else{
...
function delete_notice(){
	extract($_POST);
	$delete = $this->db->query("DELETE FROM notice where id = ".$id);
	if($delete){
		return 1;
	}
}
...
function save_stories(){
	...
	if(empty($id)){
		$data .= ", user_id = '{$_SESSION['login_id']}' ";
		$save = $this->db->query("INSERT INTO stories set ".$data);
	}else{
		$save = $this->db->query("UPDATE stories set ".$data." where id=".$id);
	}
	if($save)
		return 1;
}
