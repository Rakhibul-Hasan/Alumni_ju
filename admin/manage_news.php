...
<?php
if(isset($_GET['id'])){
$qry = $conn->query("SELECT * FROM news where id= ".$_GET['id']);
			...
			<div class="card-body">
				<form action="" id="manage-news">
					...
					<div class="form-group row">
						<div class="col-md-5">
							<label for="" class="control-label">News Title</label>
...
<script>
  ...
  var drop;
  addnewsHandler(window, 'load', function() {
    ...
    addnewsHandler(drop, 'dragover', cancel);
    addnewsHandler(drop, 'dragenter', cancel);

    addnewsHandler(drop, 'drop', function(e) {
      e = e || window.news; // get window.news if e argument missing (in IE)   
      if (e.prnewsDefault) {
        e.prnewsDefault();
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        //attach news handlers here...

        reader.readAsDataURL(file);
        addnewsHandler(reader, 'loadend', function(e, file) {
          var bin = this.result;
        ...
        }.bindTonewsHandler(file));
      }
      return false;

    });

    Function.prototype.bindTonewsHandler = function bindTonewsHandler() {
      var handler = this;
...
function addnewsHandler(obj, evt, handler) {
  if (obj.addnewsListener) {
    // W3C method
    obj.addnewsListener(evt, handler, false);
  } else if (obj.attachnews) {
    // IE method.
    obj.attachnews('on' + evt, handler);
  } else {
    // Old school method.
    obj['on' + evt] = handler;
  }
