    ...
    <div class="container-fluid h-100">
        <div class="row h-100 align-items-center justify-content-center text-center">
            <div class="col-lg-8 align-self-end mb-4 page-title">
                <h3 class="text-dark">Help Section</h3>
                <hr class="divider my-4" />
                <div class="row col-md-12 mb-2 justify-content-center">
                    <button class="btn btn-primary btn-block col-sm-3" type="button" id="new_help"><i
                            class="fa fa-plus"></i> Seek Help</button>
                </div>
            </div>
                ...
                <div class="col-md-10">
                    <div class="input-group mb-1">
                        ...
                        <input type="text" class="form-control" id="filter"
                            placeholder="Search help posts" aria-label="Filter"
...
<div class="container pt-2">
    <?php
    $event = $conn->query("SELECT f.*,u.name from help_topics f inner join users u on u.id = f.user_id order by f.id desc");
                ...
                <div class="row  align-items-center justify-content-center text-center h-100">
                    <div class="">
                        <?php if ($_SESSION['login_id'] == $row['user_id']): ?>
                            <div class="dropdown float-right mr-4">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    <a class="dropdown-item edit_help" data-id="<?php echo $row['id'] ?>"
                                        href="javascript:void(0)">Edit</a>
                                    <a class="dropdown-item delete_help" data-id="<?php echo $row['id'] ?>"
                        ...
                        <span class="badge badge-info float-left px-3 pt-1 pb-1">
                            <b><i>Posted by: <span class="filter-txt">
                                        <?php echo $row['name'] ?>
                                    </span></i></b>
                        </span>
...
<script>
    ...
    $('.view_topic').click(function () {
        location.replace('index.php?page=view_help&id=' + $(this).attr('data-id'))
    })
    $('.edit_help').click(function () {
        uni_modal("Edit Topic", "manage_help.php?id=" + $(this).attr('data-id'), 'mid-large')
    })
    ...
    function delete_help($id) {
        start_load()
        $.ajax({
            url: 'admin/ajax.php?action=delete_help',
            method: 'POST',
            data: { id: $id },
