			...
			<div class="card-body">
				<form action="" id="manage-stories">
					...
					<div class="form-group row">
						<div class="col-md-5">
							<label for="" class="control-label">Story Title</label>
							<input type="text" class="form-control" name="title"  value="<?php echo isset($title) ? $title :'' ?>" required>
						</div>
					</div>
					<div class="form-group row">
						<div class="col-md-5">
							<label for="" class="control-label">Story Writer</label>
							<input type="text" class="form-control" name="writer"  value="<?php echo isset($writer) ? $writer :'' ?>" required>
						</div>
					</div>
...
<script>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					...
					setTimeout(function(){
						location.href = "index.php?page=story"
					},1500)
  ...
  var drop;
  addstoriesHandler(window, 'load', function() {
    ...
    addstoriesHandler(drop, 'dragover', cancel);
    addstoriesHandler(drop, 'dragenter', cancel);

    addstoriesHandler(drop, 'drop', function(e) {
      e = e || window.stories; // get window.stories if e argument missing (in IE)   
      if (e.prstoriesDefault) {
        e.prstoriesDefault();
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        //attach stories handlers here...

        reader.readAsDataURL(file);
        addstoriesHandler(reader, 'loadend', function(e, file) {
          var bin = this.result;
        ...
        }.bindTostoriesHandler(file));
      }
      return false;

    });

    Function.prototype.bindTostoriesHandler = function bindTostoriesHandler() {
      var handler = this;
...
function addstoriesHandler(obj, evt, handler) {
  if (obj.addstoriesListener) {
    // W3C method
    obj.addstoriesListener(evt, handler, false);
  } else if (obj.attachstories) {
    // IE method.
    obj.attachstories('on' + evt, handler);
  } else {
    // Old school method.
    obj['on' + evt] = handler;
  }
